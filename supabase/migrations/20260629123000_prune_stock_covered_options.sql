-- Final option-generation cleanup.
-- Replaces only calculate_ctp_options and adjusts the quote_options strategy check.
-- No tables are dropped and no operational data is deleted.

alter table public.quote_options
  drop constraint if exists quote_options_sourcing_strategy_check;

alter table public.quote_options
  add constraint quote_options_sourcing_strategy_check
  check (sourcing_strategy in (
    'current_stock',
    'existing_open_po',
    'stock_plus_open_po',
    'new_standard_po',
    'secondary_standard_vendor',
    'fast_track',
    'air_freight',
    'combined_supply'
  ));
